...
import { transactionFingerprint, investmentActivityFingerprint } from '../../src/lib/fingerprint.js';

describe('transactionFingerprint', () => {
  it('produces a 64-char hex hash', () => {
    const fp = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    expect(fp).toMatch(/^[a-f0-9]{64}$/);
  });

  it('is deterministic for same inputs', () => {
    const a = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    const b = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    expect(a).toBe(b);
  });

  it('differs when account changes', () => {
    const a = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    const b = transactionFingerprint('acc-2', '2025-01-15', -50.00, 'Coffee Shop');
    expect(a).not.toBe(b);
  });

  it('differs when date changes', () => {
    const a = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    const b = transactionFingerprint('acc-1', '2025-01-16', -50.00, 'Coffee Shop');
    expect(a).not.toBe(b);
  });
  ...
  it('differs when description changes', () => {
    const a = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Coffee Shop');
    const b = transactionFingerprint('acc-1', '2025-01-15', -50.00, 'Tea Shop');
    expect(a).not.toBe(b);
  });
});
